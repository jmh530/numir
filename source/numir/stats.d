...
unittest
{
    ...
    cdouble[] x = [1.0 + 2i, 2 + 3i, 3 + 4i, 4 + 5i];
    cdouble result = 2.5 + 3.5i;
...
template var(sumTemplateArgs...)
{
    ...
    Params:
        slice = input slice
        isPopulation = true if computing population variance, false otherwise (default)
    Returns:
        variance of slice
    +/
...
unittest
{
    ...
    assert(approxEqual(x.sliced.var, 54.76563 / 11));
...
unittest
{
    ...
    assert(approxEqual(x.sliced(2, 6).var, 54.76563 / 11));
